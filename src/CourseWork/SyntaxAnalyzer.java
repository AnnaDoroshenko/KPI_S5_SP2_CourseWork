...
public class SyntaxAnalyzer {
    ...
    public boolean parse(List<Token> listOfTokens) {
        ...
        boolean ifStatementMet = false;
        boolean ifStatementFinished = false;
        int ifStatementShift = -1;
        int ifStatementEndLabelIndex = -1;
        int ifStatementGlobalEnd = -1;
...
//                System.out.println("yeaaa " + lineIndex);
    ...
    // private int findPattern(List<Piece> line, PieceName[] pattern) {
    //     for (int i = 0; i < line.size() - pattern.length; i++) {
    //         boolean matches = true;
    //         for (int j = 0; j < pattern.length; j++) {
    //             if (line.get(i + j).name != pattern[j]) {
    //                 matches = false;
    //                 break;
    //             }
    //         }
    ...
    // The type of the reduced pattern can be found with the type of the operator return
    // private Piece reduceExpression(List<Piece> line) {
    //     final List<PieceName[]> operatorPatterns = new ArrayList<>();
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Plus, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Minus, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Star, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Slash, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Greater, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.GreaterOrEquals, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Less, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.LessOrEquals, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Number, PieceName.Equals, PieceName.Number});
    //     operatorPatterns.add(new PieceName[] {PieceName.Bool, PieceName.Equals, PieceName.Bool});
    //     operatorPatterns.add(new PieceName[] {PieceName.Bool, PieceName.And, PieceName.Bool});
    //     operatorPatterns.add(new PieceName[] {PieceName.Bool, PieceName.Or, PieceName.Bool});
    //
    //     // final PieceName[] patternComma = {PieceName.Number, PieceName.Minus, PieceName.Number};
    //     final List<PieceName[]> parenthesesPatterns = new ArrayList<>();
    //     parenthesesPatterns.add(new PieceName[] {PieceName.ParenthesesOpen, PieceName.Number, PieceName.ParenthesesClose});
    //     parenthesesPatterns.add(new PieceName[] {PieceName.ParenthesesOpen, PieceName.Bool, PieceName.ParenthesesClose});
    //
    //     boolean progressMade;
    //     do {
    //         progressMade = false;
    //
    //         for (PieceName[] pattern : parenthesesPatterns) {
    //             final int parenthesesPatternStart = findPattern(line, pattern);
    //             if (parenthesesPatternStart != -1) {
    //                 if (parenthesesPatternStart > 0 &&
    //                         (line.get(parenthesesPatternStart - 1).name == PieceName.FunctionBool ||
    //                          line.get(parenthesesPatternStart - 1).name == PieceName.FunctionNumber)) {
    //                     // The parentheses are from function call => don't reduce
    //                     continue;
    //                 }
    //
    //                 // TODO: apply pattern
    //                 progressMade = true;
    //                 break; // allow one pattern application at a time
    //             }
    //         }
    //         if (progressMade) {
    //             continue;
    //         }
    //
    //         for (PieceName[] pattern : operatorPatterns) {
    //             final int operatorPatternStart = findPattern(line, pattern);
    //             if (operatorPatternStart != -1) {
    //                 // Apply pattern
    //                 final Piece pieceA = line.remove(operatorPatternStart);
    //                 final Piece pieceOperator = line.remove(operatorPatternStart);
    //                 final Piece pieceB = line.remove(operatorPatternStart);
    //                 // list.add(operatorPatternStart, new Piece(getOperatorResultType(pieceOperator),
    //                 //             pieceB.content +
    //                 //             "push eax\n" +
    //                 //             pieceA.content +
    //                 //             "pop ebx\n" +
    //                 //             pieceOperator.content
    //                 //             ));
    //
    //                 progressMade = true;
    //                 break; // allow one pattern application at a time
    ...
    //         }
    //
    //         // If we've reached here => nothing regular left to do
    //
    //     } while (progressMade);
    //
    //     if (line.size() == 1) {
    //         return line.get(0);
    //     } else {
    //         // Error: could not reduce the expression => some error inside
    //         return null;
    //     }
    // }
    ...
    // public boolean reduceFunctionIntoExpression(List<Token> line, int startIndex) {
    //     // startIndex = index of ID
    //     assert(startIndex == id of a function);
    //     assert(startIndex + 1 == '(');
    //
    ...
    // checks whether there aren't any '(' or ')' right in from or right after each ','
    // private boolean areCommasCorrect(List<Token> line) {
    //     boolean lastWasParentheses = false;
    //     boolean lastWasComma = false;
    //     for (Token token : line) {
    //         final TokenName currentTokenName = token.getTokenName();
    //         if (currentTokenName == TokenName.COMMA) {
    //             if (lastWasParentheses) {
    //                 return false;
    //             }
    //
    //             lastWasComma = true;
    //         } else if (currentTokenName == TokenName.LEFT_PARENTHESIS || currentTokenName == TokenName.RIGHT_PARENTHESIS) {
    //             if (lastWasComma) {
    //                 return false;
    //             }
    //
    //             lastWasParentheses = true;
    //         } else {
    //             lastWasParentheses = false;
    //             lastWasComma = false;
    //         }
    //     }
    //
